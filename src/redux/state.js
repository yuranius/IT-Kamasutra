...
export let addPost = (postImage) => {
    let arrPosts = state.createPostsPage.posts.slice(-1)[0].id; //получаем id последнего элемента массива

    let newPost = {
        id: arrPosts+1,
        postview: state.createPostsPage.newPostText,
        postimage:postImage,
    }
    state.createPostsPage.posts.push(newPost);
    state.createPostsPage.newPostText = '';
...
export let updateNewPostText = (newText) => {
    state.createPostsPage.newPostText = newText;
    RenderEntireFree(state, addPost, updateNewPostText);
